...
function Arp () {
  ...
  this._pcapFilter = 'arp';
  this._pcapSession = undefined;
...
Arp.prototype.init = function(interface) {
  ...
  // pcap session
  this._pcapSession = pcap.createSession(interface, this._pcapFilter);
  // event handlers
  this._pcapSession.on('packet', function (rawPacket) {
...
Arp.prototype.reply = function(targetProtoAddress,
  ...
  var arpBuffer = buffer.createBuffer();
  arpBuffer.setMacDestination(targetHardwareAddress);
  arpBuffer.setMacSource(this._getHardwareAddress());
  // ..
  arpBuffer.setSourceHardwareAddress(this._getHardwareAddress());
...
Arp.prototype.sendPacket = function(packet) {
   ...
   */
  this._pcapSession.inject(new Buffer(packet));
};
